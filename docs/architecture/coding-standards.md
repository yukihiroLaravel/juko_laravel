...
実装は TDD で進める。失敗するテストを先に書き、それを通す形で実装する。

| 段 | 内容 |
|---|---|
| 起票 | 変更の範囲と受け入れ基準を決める。書く前に依頼者の承認を取る |
| Red | 失敗するテストを書く。書いたあと、実装に入る前に依頼者の承認を取る |
| Green | テストを通す最小の実装を書く |
| Refactor | 規約に合わせて整える |
| 評価 | 検査・静的解析・全テストを通す |

承認を取る場所が2つあるのは、取り違えを直す費用が段を追うごとに上がるためである。Red の段が仕様の確定点になる。ここを承認できれば、取り違えは実装前に止まる。

検査と静的解析は規約に合っているかだけを見ており、振る舞いが正しいかは見ていない。両者を混同しない。

## テスト

- 対象の Controller や Service の配置に対応したサブディレクトリに `{行う処理}Test.php` で作成する
- AAA パターンを利用し、必ずコメントで `Arrange`・`Act`・`Assert` を記述する
- 対応する受け入れ基準があるときは、テスト関数の直前に AC-ID をコメントで書く。関数名は日本語のまま保つ

```php
/** AC-ENROLL-002 */
public function test_同じ受講生を同じ講座に二重登録できない(): void
```

仕様は `docs/specs/`・起票・テストの3か所に分かれる。ID を書いておくと `grep -rn 'AC-ENROLL-002'` で3つを同時に引ける。書かないと、仕様を変えたときにどのテストを直すべきかを検索で辿れない。
