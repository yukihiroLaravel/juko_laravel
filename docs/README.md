...
業務知識は稼働中の実装から復元し、業務側の確認を受けている（復元 2026-08-17・確認 2026-08-20）。システム仕様と構成は実装を読み直して整理したものであり、現在の実装の振る舞いをそのまま記述している。

実装が業務ルールに沿っていない箇所は未確定事項として切り出している。着手する前に目を通す。
