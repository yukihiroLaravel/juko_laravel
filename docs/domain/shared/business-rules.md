...
ここに書かれたルールは、稼働中の実装から復元したものである（整理日 2026-08-17）。業務側への確認を経ていないため、実装が業務判断とずれている可能性がある。ずれの疑いがあるものは `docs/domain/open-questions.md` に確認事項として切り出している。個別の出典欄は、上記と異なる出所を持つルールにだけ記載する。
