...
| 段 | 確定するもの | まだ確定しないもの |
|---|---|---|
| 2 起票 | やること・やらないこと。業務ルールの確定。未確定事項の解消 | 受け入れ基準の文言。実装の詳細 |
| 4 関門2 | 何を確かめるか（仕様の確定点） | — |
| 5 実装 | 受け入れ基準の ID（新設する場合） | 受け入れ基準の文言 |
| 8 反映 | 受け入れ基準の文言 | — |

受け入れ基準の文言を段2で書かないのは、関門2で差し戻されると誤った記述が恒久文書に残るためである。逆に業務ルールの確定と未確定事項の解消を段2で行うのは、それらが関門1で承認済みの判断であり、テストの内容によって覆らないためである。
...
- `docs/domain/open-questions.md` を見る。依頼の主題が未確定として記録済みかもしれない。記録があったときは次で分ける
  ...
  - 依頼が暫定方針を覆す方向 → 関門1の提示に、その矛盾と未確定事項の ID を含める。新しい関門を立てない
...
この段で恒久文書へ反映するのは、業務ルールの確定と、未確定事項の解消だけである。どちらも関門1で承認済みの判断であり、これから書くテストの内容では覆らない。実装後に回すと、起票を消すときに判断の背景ごと失われる。

受け入れ基準の文言はこの段では書かない。段5と段8で扱う。該当がなければ起票の反映欄に「該当なし」と記す。
...
この段で扱うのは、受け入れ基準の文言と、データ構造・エンドポイントの記述である。業務ルールと未確定事項は段2で済ませてあるので、ここでは触らない。
