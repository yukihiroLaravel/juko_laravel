...
起票（`docs/changes/`）は上の表に含めない。恒久的に残す内容は反映してから起票ファイルごと削除する。反映する時機は内容によって違う。

| 内容 | 反映する時機 |
|---|---|
| 業務ルール・用語・未確定事項・意思決定の記録 | 着手の初期 |
| 受け入れ基準・データ構造・エンドポイント | 実装が固まってから |

前者はテストの内容では覆らない判断なので、早く書いても誤りにならない。後者を早く書くと、テストの承認で差し戻されたときに誤った記述が恒久文書に残る。
